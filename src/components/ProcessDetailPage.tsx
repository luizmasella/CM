...
export default function ProcessDetailPage() {
  ...
  return (
    <div className="container mx-auto px-4 py-8 space-y-6">
      <div className="bg-white rounded-lg shadow-md p-4"><button onClick={closeProcessPage} className="flex items-center gap-2 text-blue-600 hover:text-blue-800"><ChevronRight size={20} className="rotate-180" />Voltar</button></div>
      <div className="bg-gradient-to-r from-blue-600 to-blue-700 rounded-xl shadow-xl p-6 text-white">
        <div className="flex justify-between items-start">
            <div><h1 className="text-3xl font-bold mb-2">{localPericia.numeroProcesso}</h1><div className="flex items-center gap-4 text-blue-100"><div className="flex items-center gap-2"><User size={18} /><span>{localPericia.reclamante}</span></div><div className="flex items-center gap-2"><Calendar size={18} /><span>{new Date(localPericia.data).toLocaleDateString('pt-BR')}</span></div></div></div>
            <div className="flex gap-2">
                {!isEditing ? <button onClick={() => setIsEditing(true)} className="bg-white/20 hover:bg-white/30 px-4 py-2 rounded-lg flex items-center gap-2"><Edit2 size={18} /> Editar</button> : (
                    <><button onClick={saveChanges} className="bg-green-500 hover:bg-green-600 px-4 py-2 rounded-lg"><CheckCircle size={18} /> Salvar</button><button onClick={() => { setLocalPericia(currentPericia); setIsEditing(false); }} className="bg-red-500 hover:bg-red-600 px-4 py-2 rounded-lg"><X size={18} /> Cancelar</button></>
                )}
            </div>
        </div>
      </div>
